...
```{r}
qual = c(2, 3, 4, 5, 7, 8, 9, 11, 16, 17)
quant = c(1, 6, 10, 12, 13, 14)
data_ = data[2:18]
res.pca = PCA(data_, scale.unit=TRUE, ncp=5, quali.sup=qual, graph=F)
```
...
```{r}
library(MASS)
quant = c(1, 6, 10, 12, 13, 14,17)
data_q = data_[, quant]
(model <- lda(y~., data = data_q))
```
