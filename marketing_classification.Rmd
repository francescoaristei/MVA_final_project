...
As shown in the above plot, 'duration' and 'campaign' variables allow to create aproximately 2 clusters of customers:
...
With a sound knowledge of the distribution of key variables, further analysis of each customer characteristic can be carried out to investigate its influence on the subscription rate, i.e. on the rate of subscriptions to the term deposit.
...
```{r}
data_dummy <- dummy_cols(data_dummy, select_columns = c('job','marital', 'education', 'default', 'housing', 'loan', 'month'), remove_selected_columns = TRUE)
```
...
data_dummy_scaled$pdays = normalize(data_dummy_scaled$pdays)
